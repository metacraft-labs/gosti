...
#   RP5C1_OUT   scratch dir for build output (default: the gitignored build/)
...
if [ -z "${REPRO_ROOT:-}" ]; then
  if [ -d "${VMH_ROOT}/../reprobuild" ]; then
    ...
    echo "build-rp5c1.sh: cannot find the reprobuild checkout." >&2
...
OUT="${RP5C1_OUT:-${VMH_ROOT}/build/rp5c1-out}"
mkdir -p "${OUT}"
