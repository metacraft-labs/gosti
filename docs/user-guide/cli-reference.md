...
- **incus:** `run --ephemeral --backend incus --baseline <container> [--base-image <alias>] [--user-data <file>] [--incus-security-nesting] [--incus-nested-kvm] -- <probe cmd>`. Default base image `vmh-base`; `--user-data` injects cloud-init (the JIT seam); the probe defaults to `true`. The two capability flags are trusted-controller options configured before first start; nested KVM implies nesting.
...
The two Incus capability flags are accepted only with
`run --ephemeral --backend incus`. They are host privileges selected by a local
Incus administrator or by the authenticated `vm-harness serve` fleet
controller. They must never be derived from workflow or guest user-data. Their
device path, type, and mode are intentionally not parameterized.
